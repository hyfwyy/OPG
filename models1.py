...
class TransformerMapper(nn.Module):
    ...
    def __init__(self, dim_clip: int, dim_embedding: int, prefix_length: int, clip_length: int, num_layers: int = 8):
        ...
        self.prefix_const = nn.Parameter(torch.randn(clip_length, dim_embedding), requires_grad=True)
...
class ClipCaptionModel(nn.Module):
    ...
    #     super(ClipCaptionModel, self).train(mode)
    #     self.gpt.eval()
    #     return self
    ...
    def __init__(self, prefix_length: int, clip_length: Optional[int] = None, prefix_size: int = 1024,
        ...
        self.weight = torch.nn.init.xavier_normal_(torch.zeros((batch_size,prefix_length)))
        if mapping_type in [MappingType.MLP,'mlp']:
            # origin
